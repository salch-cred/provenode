        ...
        <div className="card">
          ...
          {showForm && <div style={{padding:16,borderBottom:'2px solid var(--border)'}}>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
              <div className="form-group"><label className="form-label">Device ID</label><input value={id} onChange={e=>setId(e.target.value)} placeholder="CAM-SIN-042" /></div>
              <div className="form-group"><label className="form-label">Type</label><select value={type} onChange={e=>setType(e.target.value)}><option>camera</option><option>drone</option><option>robot</option><option>vehicle</option></select></div>
              <div className="form-group"><label className="form-label">Architecture</label><select value={arch} onChange={e=>setArch(e.target.value)}><option>arm64</option><option>x64</option><option>armv7</option></select></div>
              <div className="form-group"><label className="form-label">Location</label><input value={loc} onChange={e=>setLoc(e.target.value)} placeholder="Singapore" /></div>
              <div className="form-group"><label className="form-label">Fleet</label><input value={fleet} onChange={e=>setFleet(e.target.value)} placeholder="production" /></div>
            </div>
          ...
          <tbody>{!devices.length ? <tr><td colSpan={6} className="empty">No devices registered.</td></tr> :
