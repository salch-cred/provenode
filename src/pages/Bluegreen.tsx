    ...
    <div style={{display:'grid',gridTemplateColumns:'1fr 340px',gap:20}}>
      ...
      <div className="card"><div className="card-header"><span className="card-title">Configure B/G</span></div>
        <div className="card-body">
          <div className="form-group"><label className="form-label">Project ID</label><input className="form-input" value={projectId} onChange={e=>setProjectId(e.target.value)} placeholder="vision-edge-prod" /></div>
          ...
          <div className="form-group"><label className="form-label">Blue deployment</label><select className="form-input" value={blueId} onChange={e=>setBlueId(e.target.value)}><option value="">—</option>{deps.map(d=><option key={d.id} value={d.id}>{d.model} v{d.version}</option>)}</select></div>
          <div className="form-group"><label className="form-label">Green deployment</label><select className="form-input" value={greenId} onChange={e=>setGreenId(e.target.value)}><option value="">—</option>{deps.map(d=><option key={d.id} value={d.id}>{d.model} v{d.version}</option>)}</select></div>
