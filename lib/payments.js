...
export async function createPaymentIntent({ item, itemId, payer, receiver, description, amountShelbyUSD }) {
...
export async function getPaymentIntent(id) {
  const raw = await getDB().get(`pay:${id}`);
  return raw ? JSON.parse(raw) : null;
}

export async function markIntentPaid(id, { txHash, sender, micropaymentBcs }) {
  const db = getDB();
 ...
 */
export async function findOrCreateIntent({ resourceKey, item, itemId, payer, receiver, description }) {
  const db = getDB();
  const pointerKey = `paywall:${resourceKey}:${payer || 'anon'}`;
  const existingId = await db.get(pointerKey);
  if (existingId) {
    try {
      const intent = await getPaymentIntent(JSON.parse(existingId));
  ...
  const intent = await createPaymentIntent({ item, itemId, payer, receiver, description });
  await db.put(pointerKey, JSON.stringify(intent.id));
  return intent;
}

/** Look up the paywall intent for a resource + payer without creating one. */
export async function getPaywallIntent(resourceKey, payer) {
  const raw = await getDB().get(`paywall:${resourceKey}:${payer || 'anon'}`);
  if (!raw) return null;
  try { return await getPaymentIntent(JSON.parse(raw)); } catch { return null; }
}
