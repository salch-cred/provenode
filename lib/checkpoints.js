...
export function buildResumeCommand(checkpoint, shelbyAddress) {
  const rpcBase = process.env.SHELBY_NETWORK === 'testnet' ? 'https://api.testnet.shelby.xyz' : 'https://api.shelbynet.shelby.xyz';
  const downloadUrl = `${rpcBase}/shelby/v1/blobs/${shelbyAddress}/${checkpoint.shelbyObjectId.split('/').slice(-2).join('/')}`;
  return {
