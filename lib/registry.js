...
/**
 * Canonical explorer URL builder — SINGLE SOURCE OF TRUTH.
 *
 * Shelbynet is an Aptos-compatible chain without a dedicated first-party
 * explorer UI for arbitrary txns, so we point at the Aptos Explorer in
 * custom-network mode against the Shelbynet RPC. Every explorer link in the
 * codebase MUST come from here — previously three modules and the landing page
 * each hardcoded their own variant, and the README cited a fourth host.
 */
export function explorerUrl(kind, id, rpc = SHELBY_RPC) {
  const path = kind === 'account' ? 'account' : 'txn';
  return `https://explorer.aptoslabs.com/${path}/${id}?network=custom&customNetworkUrl=${encodeURIComponent(rpc)}`;
}

export const txExplorerUrl = (hash, rpc = SHELBY_RPC) => explorerUrl('txn', hash, rpc);
export const accountExplorerUrl = (address, rpc = SHELBY_RPC) => explorerUrl('account', address, rpc);
...
export async function getTxByVersion(version) {
  ...
  return {
    ...
    explorerUrl: txExplorerUrl(t.hash),
  };
}
