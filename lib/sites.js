...
/**
 * CI deploy keys — site-scoped bearer tokens for GitHub Actions.
 * Format: pvnd_<random>. Stored (KV) as siteKey:<token> -> { siteId, createdAt }
 * so a leaked key can only deploy one site and can be revoked by deleting
 * the KV entry. Accepts either the raw token or "Bearer <token>".
 */
...
export function normalizeDeployKey(raw) {
