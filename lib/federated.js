...
export function weightedFedAvg(gradients, sampleCounts) {
  if (!gradients.length) throw new Error('At least one gradient is required.');
  if (gradients.length !== sampleCounts.length) throw new Error('gradients and sampleCounts must have the same length.');
  // All gradients must describe the same parameter vector, otherwise the
  // average silently reads past the end of the shorter arrays and yields NaN.
  const len = gradients[0].length;
  if (gradients.some(g => g.length !== len)) {
    throw new Error('All device gradients must have the same length.');
  }
  const totalSamples = sampleCounts.reduce((a, b) => a + b, 0);
  if (!(totalSamples > 0)) throw new Error('Total sample count must be greater than zero.');
  const averaged = new Float32Array(len);
