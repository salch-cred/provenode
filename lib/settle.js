...
export async function settleMicropayment({ intent, micropaymentBcs, sender, env = process.env }) {
  ...
  const paid = await markIntentPaid(intent.id, { txHash, sender, micropaymentBcs });
  await logAudit('payment.settled', { actor: sender || mp.sender.toString(), target: intent.itemId, details: { intentId: intent.id, amountMicro: intent.amountMicro, txHash } });
